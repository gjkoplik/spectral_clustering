# Gary Koplik
# Fall, 2017
# swissroll_plot_kmeans.R

# this code loads in the swiss roll data set
#   is currently coded to implement K-means clustering using the stats package K-means algorithm
...
dat$color <- factor(kmeans(dat, centers = num)$cluster)
...
for(i in seq(0.0, 6.3, by = 0.1)){
  ...
  remDr <- rD$client
  if(10*i < 10){
    export(graph, file = paste("km_",0, 10*i + 1,
                               ".png", sep = ""),
           selenium = rD)
  }
  if (10*i >= 10){
    export(graph, file = paste("km_", 10*i + 1,
                               ".png", sep = ""),
           selenium = rD)    
  }
...
# once you've made that gif, this will make it run smoothly

frames <- image_read("./animation_kmeans.gif")

animation <- image_animate(frames[3:63], fps = 10)
image_write(animation, "./swiss_roll_kmeans_animate.gif")
